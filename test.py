...
# 실제 content_type 키 맞추기
real_type = "책" if "책" in content_type else "영화"

if genre and real_type:
    st.subheader(f"👉 {genre} 장르의 {real_type} 추천 3선 🌟")
    choices = random.sample(recommendations[genre][real_type], 3)
    for idx, choice in enumerate(choices, 1):
        st.markdown(f"## {idx}. {choice['제목']} {'📖' if real_type=='책' else '🎬'}")
        st.image(choice["이미지"], width=250)
